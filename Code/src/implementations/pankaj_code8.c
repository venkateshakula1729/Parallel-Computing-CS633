...
// Optimized data distribution using MPI_Scatterv and improved memory access patterns
// 1. Replace point-to-point communication with `MPI_Scatterv`
// 2. Optimize memory access patterns with `memcpy` for block operations
double* distributeData(int rank, const SubDomain* subdomain, double* globalData,
    ...
    // Calculate local data size for this process
    ...
    // Arrays needed for MPI_Scatterv
    int numProcs = pX * pY * pZ;
    int* sendcounts = NULL;    // How many elements to send to each process
    int* displs = NULL;        // Displacement from start of send buffer to each process's data
    double* sendBuffer = NULL; // Buffer containing data for all processes in the order needed by scatterv

    if (rank == 0) {
        // Root process prepares the data for scattering
        ...
        // First pass: calculate size for each process and total buffer size
        int totalBufferSize = 0;
        for (int p = 0; p < numProcs; p++) {
            SubDomain procDomain;
            calculateSubDomainBoundaries(p, pX, pY, pZ, nX, nY, nZ, &procDomain);

            sendcounts[p] = procDomain.tempWidth * procDomain.tempHeight *
                           procDomain.tempDepth * timeSteps;
            displs[p] = totalBufferSize;
            totalBufferSize += sendcounts[p];
        }

        // Allocate send buffer to hold all processes' data
        sendBuffer = (double*)malloc(totalBufferSize * sizeof(double));
        if (!sendBuffer) {
            printf("Rank 0: Failed to allocate send buffer\n");
            free(localData);
            free(sendcounts);
            free(displs);
            return NULL;
        ...
        // Second pass: fill the send buffer with data for each process
        for (int p = 0; p < numProcs; p++) {
            SubDomain procDomain;
            calculateSubDomainBoundaries(p, pX, pY, pZ, nX, nY, nZ, &procDomain);
            ...
            // Iterate through the subdomain for this process
            for (int z = procDomain.tempStartZ; z <= procDomain.tempEndZ; z++) {
                for (int y = procDomain.tempStartY; y <= procDomain.tempEndY; y++) {
                    // Use memcpy to copy entire rows at once for better cache performance
                    int rowSize = (procDomain.tempEndX - procDomain.tempStartX + 1) * timeSteps;
                    int globalStartIdx = (getLinearIndex(procDomain.tempStartX, y, z, nX, nY, nZ)) * timeSteps;

                    // Copy the entire row of time series data at once
                    memcpy(&sendBuffer[bufIdx], &globalData[globalStartIdx], rowSize * sizeof(double));
                    bufIdx += rowSize;
                }
            }
        }
    ...
    // Use MPI_Scatterv to distribute data in one collective operation
    MPI_Scatterv(
        sendBuffer,                // Send buffer (only used at root)
        sendcounts,                // Array specifying how many elements to send to each process
        displs,                    // Array of displacements from send buffer to each process's data
        MPI_DOUBLE,                // Data type
        localData,                 // Receive buffer for this process
        localDataSize,             // How many elements this process receives
        MPI_DOUBLE,                // Data type
        0,                         // Root process
        MPI_COMM_WORLD             // Communicator
    );

    // Clean up resources used by root process
    if (rank == 0) {
        free(sendBuffer);
        free(sendcounts);
        free(displs);
